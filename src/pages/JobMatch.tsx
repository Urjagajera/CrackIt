...
export default function JobMatch() {
  ...
  const [resumes, setResumes] = useState<any[]>(mockResumes);
  const [selectedResumeId, setSelectedResumeId] = useState<string>(mockResumes[0].id.toString());
  ...
  const handleCalculate = async (e: FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const data = await apiFetch<{ message: string; match: any }>('/job-match', {
        method: 'POST',
        body: JSON.stringify({
          jd_text: cleanJD,
      ...
      const newScore = match.match_score || 85;
      const gaps = match.gap_analysis_json || {};
      ...
      showToast(err.message || 'Failed to calculate job match.', 'error');
    } finally {
      setIsLoading(false);
    }
  };

  const missingSkills: string[] = gapAnalysis?.missing_skills || [
    'Kubernetes / Helm Charts',
    'Apache Kafka Event Pipelines',
    'System Scalability Metrics',
  ];

  const recommendations: string[] = gapAnalysis?.recommendations || [
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-gutter">
        ...
        <section className="col-span-12 lg:col-span-7 flex flex-col gap-gutter">
          ...
          <form
            ...
            className="bento-card bg-surface-container-lowest p-6 rounded-[24px] shadow-[0_10px_30px_rgba(65,81,187,0.04)] border border-surface-variant/30 flex flex-col justify-between flex-1"
          >
            <div>
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-label-sm text-outline-variant font-medium text-xs">Paste Plain Text or URL</span>
              </div>
              ...
              <textarea
                ...
                placeholder="Paste the target job description (responsibilities, requirements, technical stack) or URL..."
            ...
            <div className="mt-6 flex justify-end">
              ...
              >
                ...
                <span className="material-symbols-outlined">{isLoading ? 'sync' : 'bolt'}</span>
              </button>
            </div>
          </form>
        </section>
            ...
            <div className="space-y-gutter">
              ...
              <div className="bento-card bg-surface-container-lowest p-8 rounded-[24px] shadow-[0_10px_30px_rgba(65,81,187,0.04)] border border-surface-variant/30 flex flex-col items-center justify-center text-center relative overflow-hidden">
                <div className="absolute top-0 right-0 p-4">
                  <span className="px-3 py-1 bg-tertiary-container text-on-tertiary-container text-label-sm rounded-full font-bold text-xs">
                    AI Verified
                  </span>
                </div>
                ...
                <div className="relative w-40 h-40 my-4 flex items-center justify-center">
                  ...
                  <div className="absolute flex flex-col items-center">
                    <span className="font-headline-xl text-[44px] font-extrabold text-on-surface leading-none">
                      {score}%
                    </span>
                ...
                <p className="text-body-md text-on-surface-variant text-sm">
                  {gapAnalysis?.summary || 'Your resume aligns well with core technical requirements.'}
                </p>
              </div>
              ...
              <div className="bento-card bg-surface-container-lowest p-6 rounded-[24px] shadow-[0_10px_30px_rgba(65,81,187,0.04)] border border-surface-variant/30">
                ...
                <div className="space-y-3">
                  {missingSkills.map((skill: string, idx: number) => (
                    <div
                      key={idx}
                      className="p-4 rounded-xl bg-secondary-fixed/20 border border-secondary-fixed-dim/30 flex items-start gap-3"
                    >
                      <span className="material-symbols-outlined text-secondary text-sm mt-0.5">add_circle</span>
                      <div>
                        <h5 className="font-bold text-xs text-on-surface">{skill}</h5>
                        <p className="text-[11px] text-on-surface-variant mt-0.5">
                          Flagged in target JD, missing in current resume draft.
                        </p>
                      </div>
                    </div>
                  ))}
                ...
                {recommendations.length > 0 && (
                  <div className="mt-4 pt-4 border-t border-surface-variant/30">
                    <p className="font-label-sm text-xs font-semibold text-on-surface-variant mb-2">Recommendations:</p>
                    <ul className="space-y-1">
                      {recommendations.map((rec: string, idx: number) => (
                        <li key={idx} className="text-[11px] text-on-surface-variant flex items-center gap-1.5">
